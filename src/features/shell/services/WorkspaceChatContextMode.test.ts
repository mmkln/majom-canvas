...
import { scopeWorkspaceChatContext } from './WorkspaceChatContextMode.ts';

function makeSnapshot(): WorkspaceChatCanvasSnapshot {
  return {
    canvasId: 'canvas-a',
...
describe('scopeWorkspaceChatContext', () => {
  it('returns only visible items in viewport mode', () => {
    const scoped = scopeWorkspaceChatContext(makeSnapshot(), 'viewport');
  ...
  it('keeps selected items with nearby structure in selection mode', () => {
    const scoped = scopeWorkspaceChatContext(makeSnapshot(), 'selection');
  ...
  it('returns null when selection mode has no selected items', () => {
    ...
    expect(scopeWorkspaceChatContext(snapshot, 'selection')).toBeNull();
  });
});
