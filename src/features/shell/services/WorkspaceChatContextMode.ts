...
export type WorkspaceChatContextMode =
  | 'none'
  | 'canvas'
  | 'viewport'
  | 'selection';

export const WORKSPACE_CHAT_CONTEXT_MODE_OPTIONS: Array<{
  value: WorkspaceChatContextMode;
  label: string;
}> = [
...
export function isWorkspaceChatContextMode(
  ...
  return WORKSPACE_CHAT_CONTEXT_MODE_OPTIONS.some((option) => option.value === value);
}

export function scopeWorkspaceChatContext(
...
function createScopedSnapshot(
  ...
  const elements = snapshot.elements
    ...
    .map((element): WorkspaceChatCanvasElement => ({
      ...element,
      parentId:
