...
import type { WorkspaceChatReviewFindingSeverity } from '../workspaceChatActions.ts';
import { getSelectionCluster } from './WorkspaceChatSnapshotLens.ts';
import type { WorkspaceChatToolDefinition } from './WorkspaceChatToolTypes.ts';

type WorkspaceChatAnalysisFinding = {
  code: string;
...
export const WORKSPACE_CHAT_ANALYSIS_TOOLS: WorkspaceChatToolDefinition[] = [
  {
    ...
    execute: (input, context) => {
      ...
      const findings: WorkspaceChatAnalysisFinding[] = [];
  ...
  },
  {
    ...
    execute: (input, context) => {
      ...
      const findings = resolveTargetElements(snapshot, input)
        ...
        .map(
          (element): WorkspaceChatAnalysisFinding => ({
  ...
  },
  {
    ...
    execute: (input, context) => {
      const snapshot = resolveRuntimeSnapshot(context.runtime);
      const elements = resolveTargetElements(snapshot, input);
      const byTitle = new Map<string, WorkspaceChatCanvasElement[]>();
      elements.forEach((element) => {
