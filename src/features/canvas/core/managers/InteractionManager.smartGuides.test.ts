...
describe('InteractionManager smart guides integration', () => {
  ...
  it('snaps a dragged task into equal spacing between neighbors', () => {
    const { scene, manager, movingTask } = createHarness();
    ...
    expect(
      getOverlayLines(manager, 'spacing').map((line) => ({
        axis: line.axis,
      ...
      { axis: 'y', position: 86, start: 422, end: 525 },
      { axis: 'y', position: 86, start: 797, end: 900 },
    ]);
    expect(
      manager
        .getSmartGuideOverlay()
  ...
  it('does not use spacing guides when spacing preference is disabled', () => {
    const { scene, manager, movingTask } = createHarness();
    const left = new TaskElement({ id: 'left', x: 150, y: 100 });
    const right = new TaskElement({ id: 'right', x: 900, y: 100 });
    scene.addElement(left);
  ...
  it('snaps a dragged element to the viewport center when enabled', () => {
  ...
  it('offers a container snap for a task already inside a story', () => {
    const { scene, manager, movingTask } = createHarness();
    const story = new StoryElement({
      id: 'story',
      x: 450,
