...
export const normalizeBackendRef = (
  input: Partial<BackendRef>
): BackendRef => ({
...
export const mergeStoriesDedup = (primary: Story[], secondary: Story[]): Story[] => {
...
export const matchesRef = (
  ...
  if (ref.id !== null && Number.isFinite(item.id) && item.id === ref.id) return true;
  return false;
};
