...
export class ContextMenu {
  ...
  private buildSections(detail: ContextMenuDetail): ContextMenuSection[] {
    ...
    const interactionSections = this.buildInteractionSections({
      target: element,
      sceneX,
      sceneY,
      includeCreationActions: element == null,
    });
    if (!element) {
      const sections: ContextMenuSection[] = [...interactionSections];
    ...
    sections.push(...interactionSections);
    if (!this.enableLegacyPlanningActions) {
      return sections;
    }
  ...
  private buildInteractionSections(options: {
    target: ICanvasElement | null;
    sceneX: number;
    sceneY: number;
    includeCreationActions: boolean;
  }): ContextMenuSection[] {
    ...
    if (options.includeCreationActions) {
      groups.push(
        ...(this.interactionAdapter.getCreationActions?.({
          scene: this.scene,
    ...
    groups.push(
      ...(this.interactionAdapter.getContextMenuActions?.({
        scene: this.scene,
        canvasManager: this.canvasManager,
        runtime: this.runtime,
        sceneX: options.sceneX,
    ...
    return groups
      ...
      .map((group) => ({
        ...
        items: group.actions.map((action) =>
          this.mapInteractionActionToMenuItem(action, {
            target: options.target,
            sceneX: options.sceneX,
            sceneY: options.sceneY,
            selectedElements,
  ...
  private mapInteractionActionToMenuItem(
    ...
    return {
      ...
      className:
        action.tone === 'primary'
          ? 'font-medium text-indigo-700 hover:bg-indigo-50 hover:text-indigo-800'
          : undefined,
      leading: action.icon ? this.createLeadingIcon(action.icon) : null,
      action: () => {
        if (action.disabled) {
          return 'keep-open';
        }
