...
export class MiniMap {
  ...
  private drawElements(metrics: MiniMapMetrics): void {
    ...
    sceneElements.forEach((element: any) => {
      if (typeof element?.x !== 'number' || typeof element?.y !== 'number')
        return;
      const baseColor = this.getColorByType(this.getElementKind(element));
  ...
  private getElementKind(element: unknown): string {
    if (isStructuredCanvasNode(element)) {
      return element.nodeKind;
    }
    return 'other';
  }

  private getColorByType(type: string): string {
    if (type === 'task' || type === 'exercise' || type === 'checkpoint')
      return '#0ea5e9';
    if (type === 'story' || type === 'lesson' || type === 'module')
      return '#10b981';
