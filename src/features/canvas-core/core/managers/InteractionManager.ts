...
import { isStructuredCanvasNode } from '../../elements/utils/typeGuards.ts';
...
import { DefaultCanvasAlignmentAdapter } from '../../adapters/DefaultCanvasAlignmentAdapter.ts';
import type { CanvasRuntimeSemanticsAdapter } from '../../adapters/CanvasRuntimeSemanticsAdapter.ts';

type ResizeHandleDirection = 'nw' | 'ne' | 'se' | 'sw';

type ResizableLayoutContainer<
  TChild extends IStructuredCanvasNode = IStructuredCanvasNode,
> = ICanvasLayoutContainer<TChild> & {
...
export class InteractionManager {
  ...
  // Resize state
  private resizingElement: ResizableLayoutContainer | null = null;
  private resizeDirection: ResizeHandleDirection | null = null;
  ...
  constructor(
    ...
    private readonly semanticsAdapter: CanvasRuntimeSemanticsAdapter,
    alignmentAdapter: CanvasAlignmentAdapter = new DefaultCanvasAlignmentAdapter()
  ) {
  ...
  private getStructuredNodes(): IStructuredCanvasNode[] {
    return this.scene.getElements().filter(isStructuredCanvasNode);
  }

  private getLayoutContainers(): Array<
    ICanvasLayoutContainer<IStructuredCanvasNode>
  > {
    return this.getStructuredNodes().filter((element) =>
      this.semanticsAdapter.isLayoutContainer(element)
    ) as Array<ICanvasLayoutContainer<IStructuredCanvasNode>>;
  }

  private getLayoutChildren(): IStructuredCanvasNode[] {
    return this.getStructuredNodes().filter((element) =>
      this.semanticsAdapter.isLayoutChild(element)
    );
  }

  private getResizableLayoutContainers(): ResizableLayoutContainer[] {
    return this.getLayoutContainers().filter(
      (element): element is ResizableLayoutContainer =>
        this.semanticsAdapter.canResizeElement(element) &&
        typeof element.getResizeHandleDirectionAt === 'function'
    );
  }

  private getSceneLayoutChildren<TChild extends IStructuredCanvasNode>(
    container: ICanvasLayoutContainer<TChild>
  ): TChild[] {
    return this.getLayoutChildren().filter((element): element is TChild =>
      container.acceptsLayoutChild(element)
    );
  }

  private updateLayoutAssignments(
    children: IStructuredCanvasNode[],
    dropPlans: Map<string, StoryDropPlan> = new Map()
  ): void {
    if (children.length === 0) return;
    const containers = this.getLayoutContainers();
    const prevContainerMap = this.getLayoutChildContainerMap(containers);
    const draggedChildIds = new Set(children.map((child) => child.id));
    const plannedContainerByChildId = new Map<string, string>();
    dropPlans.forEach((plan, storyId) => {
      plan.orderedTasks.forEach((child) => {
        if (!draggedChildIds.has(child.id)) return;
        plannedContainerByChildId.set(child.id, storyId);
      });
    });
    containers.forEach((container) => {
      children.forEach((child) => {
        const childId = child.id;
        if (!container.acceptsLayoutChild(child)) {
          this.removeLayoutChild(container, childId);
          return;
        }
        const plannedContainerId = plannedContainerByChildId.get(childId);
        if (plannedContainerId) {
          this.setLayoutChildMembership(
            container,
            child,
            container.id === plannedContainerId
          );
          return;
        }
        const anchor = this.getLayoutChildAnchor(child);
        this.setLayoutChildMembership(
          container,
          child,
          container.contains(anchor.x, anchor.y)
        );
      });
    });
    const nextContainerMap = this.getLayoutChildContainerMap(containers);
    if (typeof window === 'undefined') return;
    children.forEach((child) => {
      const prevContainerId = prevContainerMap.get(child.id) ?? null;
      const nextContainerId = nextContainerMap.get(child.id) ?? null;
      if (prevContainerId === nextContainerId) return;
      const nextContainer = nextContainerId
        ? (containers.find((container) => container.id === nextContainerId) ??
          null)
        : null;
      if (
        child instanceof TaskElement &&
        nextContainer instanceof StoryElement
      ) {
        emitTaskStoryLinkSet(child, nextContainer);
      }
    });
  }

  private setLayoutChildMembership(
    container: ICanvasLayoutContainer<IStructuredCanvasNode>,
    child: IStructuredCanvasNode,
    attached: boolean
  ): void {
    const current = container.getOrderedLayoutChildren();
    const hasChild = current.some((candidate) => candidate.id === child.id);
    if (attached) {
      if (!hasChild) {
        container.replaceOrderedLayoutChildren([...current, child]);
      }
      return;
    }
    if (hasChild) {
      container.replaceOrderedLayoutChildren(
  ...
  private removeLayoutChild(
    ...
    const current = container.getOrderedLayoutChildren();
    if (!current.some((child) => child.id === childId)) return;
    container.replaceOrderedLayoutChildren(
      current.filter((child) => child.id !== childId)
    );
  }
  ...
  handleMouseDown(e: MouseEvent, sceneX: number, sceneY: number): boolean {
    ...
    const structuredNodes = this.getStructuredNodes();
    let clickedItem: (ICanvasElement & IDraggable) | null = null;

    const resizableContainers = this.getResizableLayoutContainers();
    for (let i = resizableContainers.length - 1; i >= 0; i--) {
      const el = resizableContainers[i];
    ...
    if (clickedItem) {
      ...
      if (isStructuredCanvasNode(clickedItem)) {
    ...
    if (
      ...
      !structuredNodes.some((el) => el.contains(sceneX, sceneY))
    ) {
  ...
  handleMouseMove(
    ...
    if (!this.draggingItem && !this.draggingGroup && !this.resizingElement) {
      this.clearTaskDropPreviewState();
      this.clearSmartGuides();
    }
    const rawShapes = this.scene.getShapes();
    ...
    if (this.connectionService.isCreating()) {
      const tasks = structuredNodes.filter((el) =>
        this.semanticsAdapter.isLayoutChild(el)
      );
      const others = structuredNodes.filter(
        (el) => !this.semanticsAdapter.isLayoutChild(el)
      );
      connectables = [...rawShapes, ...others, ...tasks];
    } else {
      connectables = [...rawShapes, ...structuredNodes];
    }
    ...
    const stories = this.getResizableLayoutContainers();
    ...
    if (this.isRegionSelecting) {
      ...
      // gather all elements
      const rawShapes = this.scene.getShapes();
      const all = [...rawShapes, ...this.getStructuredNodes()] as Array<
        IShape | IStructuredCanvasNode
      >;
    ...
    if (this.draggingItem) {
      ...
      if (init) {
        ...
        this.updateTaskDropPlaceholders(sceneX, sceneY);
        const shouldSuppressSmartSnap =
          this.shouldSuppressSmartSnapForTaskDrop();
        if (shouldSuppressSmartSnap) {
          this.clearSmartGuides();
        } else {
    ...
    if (this.resizingElement && this.resizeDirection) {
      ...
      const layoutChildren = this.getSceneLayoutChildren(story);
      this.applyResizeLayoutPlan(story, layoutChildren, newW, newH);
      ...
      if (snap.widthDelta !== 0 || snap.heightDelta !== 0) {
        ...
        this.applyResizeLayoutPlan(
          story,
          layoutChildren,
          snappedSize.width + snap.widthDelta,
          snappedSize.height + snap.heightDelta
        );
      }
      this.scene.changes.next();
      return;
  ...
  handleMouseUp(): void {
    ...
    if (this.draggingGroup) {
      ...
      const tasks = selectedEls.filter((el): el is IStructuredCanvasNode =>
        this.semanticsAdapter.isLayoutChild(el as ICanvasElement)
      );
      const stories = this.getLayoutContainers();
      const prevStoryMap = this.getLayoutChildContainerMap(stories);
      this.updateLayoutAssignments(tasks, this.storyDropPlans);
      const affectedStories = this.getAffectedContainersForChildren(
        tasks,
        stories,
        prevStoryMap
      );
      const dragIds = new Set(initial.keys());
      const alignChanges = this.applyAutoLayoutToContainers(
        affectedStories,
        dragIds,
        this.storyDropPlans
      );
    ...
    if (this.draggingItem) {
      ...
      if (
        ...
        this.semanticsAdapter.isLayoutChild(this.draggingItem)
      ) {
        const selectedEls = this.scene.getSelectedElements();
        const tasks = selectedEls.filter((el): el is IStructuredCanvasNode =>
          this.semanticsAdapter.isLayoutChild(el as ICanvasElement)
        ...
        alignChanges = this.applyAutoLayoutToContainers(
          affectedStories,
          dragIds,
          this.storyDropPlans
        );
    ...
    if (this.resizingElement) {
      ...
      const movedTasks = this.getResizeMovedLayoutChildren();
      ...
      this.canvas.style.cursor = 'default';
      this.getLayoutContainers().forEach((s) => {
        if ('hoveredResizeHandle' in s) {
          (s as ResizableLayoutContainer).hoveredResizeHandle = null;
        }
      });
      this.clearTaskDropPreviewState();
      this.clearSmartGuides();
  ...
  handleDoubleClick(sceneX: number, sceneY: number): void {
    ...
    const structuredNodes = [...this.getStructuredNodes()];
    structuredNodes.sort((a, b) => (b.zIndex ?? 0) - (a.zIndex ?? 0));
    for (const el of structuredNodes) {
  ...
  private updateSmartGuidesForElements(
    ...
    const minGuideLength = SMART_GUIDES_MIN_LINE_LENGTH_PX / this.panZoom.scale;
    const maxGuideLength = SMART_GUIDES_MAX_LINE_LENGTH_PX / this.panZoom.scale;
  ...
  private updateTaskDropPlaceholders(sceneX: number, sceneY: number): void {
    ...
    const hasDraggedStory = draggedElementIds.some((id) => {
      ...
      return (
        !!element &&
        this.semanticsAdapter.isLayoutContainer(element as ICanvasElement)
      );
    });
    if (hasDraggedStory) {
      this.clearTaskDropPreviewState();
    ...
    const tasks = sceneElements.filter(
      (element): element is IStructuredCanvasNode =>
        this.semanticsAdapter.isLayoutChild(element)
    );
  ...
  private findTopElementAt(
    ...
    const rawShapes = this.scene.getShapes();
    const structuredNodes = [...this.getStructuredNodes()].sort((a, b) => {
      const priorityDelta =
        this.semanticsAdapter.getHitTestPriority(a) -
        this.semanticsAdapter.getHitTestPriority(b);
      if (priorityDelta !== 0) {
        return priorityDelta;
      }
      return (a.zIndex ?? 0) - (b.zIndex ?? 0);
    });
    for (let i = structuredNodes.length - 1; i >= 0; i--) {
      const node = structuredNodes[i];
  ...
  private getLayoutChildContainerMap(
    stories: Array<ICanvasLayoutContainer<IStructuredCanvasNode>>
  ): Map<string, string> {
    const map = new Map<string, string>();
    stories.forEach((story) => {
      story.getOrderedLayoutChildren().forEach((task) => {
        if (!map.has(task.id)) {
  ...
  private getAffectedContainersForChildren(
    tasks: IStructuredCanvasNode[],
    stories: Array<ICanvasLayoutContainer<IStructuredCanvasNode>>,
    prevStoryMap: Map<string, string>
  ): Array<ICanvasLayoutContainer<IStructuredCanvasNode>> {
    if (tasks.length === 0) return [];
    const nextStoryMap = this.getLayoutChildContainerMap(stories);
  ...
  private applyAutoLayoutToContainers(
    stories: Array<ICanvasLayoutContainer<IStructuredCanvasNode>>,
    ...
    const tasks = this.getLayoutChildren();
    const taskById = new Map(tasks.map((task) => [task.id, task]));
    const epsilon = 0.01;
    stories.forEach((story) => {
      const layoutTasks = this.getSceneLayoutChildren(story);
      const orderedLayoutTasks = this.storyLayoutService.getLayoutTasks(
        story,
        layoutTasks
      );
      if (orderedLayoutTasks.length === 0) return;
      const initialTaskPositions = new Map(
        orderedLayoutTasks.map((task) => [task.id, { x: task.x, y: task.y }])
      );
      const initialStory = {
        x: story.x,
        y: story.y,
      ...
      if (dropPlan) {
        ...
        story.replaceOrderedLayoutChildren(dropPlan.orderedTasks);
      } else {
        ...
        if (plan.positions.size > 0) {
          ...
          story.replaceOrderedLayoutChildren(plan.orderedTasks);
        }
      }
      if (
  ...
  private captureResizeTaskPositions<TChild extends IStructuredCanvasNode>(
    story: ICanvasLayoutContainer<TChild>
  ): void {
    this.resizeInitialTaskPositions.clear();
    const layoutChildren = this.getSceneLayoutChildren(story);
    const orderedChildren = this.storyLayoutService.getLayoutTasks(
      story,
      layoutChildren
    );
    orderedChildren.forEach((child) => {
      this.resizeInitialTaskPositions.set(child.id, { x: child.x, y: child.y });
    });
  }
  ...
  private applyResizeLayoutPlan<TChild extends IStructuredCanvasNode>(
    story: ResizableLayoutContainer<TChild>,
    ...
    const taskById = new Map(
      layoutChildren.map((child) => [child.id, child] as const)
    );
    plan.positions.forEach((pos, id) => {
      const child = taskById.get(id);
      if (!child) return;
      child.x = pos.x + offsetX;
      child.y = pos.y + offsetY;
    });
    story.replaceOrderedLayoutChildren(plan.orderedTasks);
  }
  ...
  private getResizeMovedLayoutChildren(): {
  ...
  private getLayoutChildAnchor(task: IStructuredCanvasNode): {
    x: number;
    y: number;
  } {
    return {
