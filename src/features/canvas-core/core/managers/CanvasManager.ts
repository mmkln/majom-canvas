...
type ProgressAggregateNode = IStructuredCanvasNode & {
  links: string[];
  progress: number;
};

function isProgressSourceNode(
  element: IConnectable | IStructuredCanvasNode | undefined
): element is ProgressSourceNode {
  return (
    !!element &&
    isStructuredCanvasNode(element) &&
    element.nodeKind === 'task' &&
    typeof (element as { status?: unknown }).status === 'string'
  );
}

function isProgressAggregateNode(
  element: IConnectable | IStructuredCanvasNode | undefined
): element is ProgressAggregateNode {
  return (
    !!element &&
    isStructuredCanvasNode(element) &&
    element.nodeKind === 'goal' &&
    Array.isArray((element as { links?: unknown }).links) &&
    typeof (element as { progress?: unknown }).progress === 'number'
  );
}
...
export class CanvasManager {
  ...
  private pinchElement: ICanvasLayoutContainer<IStructuredCanvasNode> | null =
    null;
  ...
  private readonly appearanceAdapter: CanvasAppearanceAdapter | null;
  ...
  private cachedStructuredNodes: IStructuredCanvasNode[] = [];
  private cachedStructuredNodesSorted: IStructuredCanvasNode[] = [];
  private cachedProgressSourceNodes: ProgressSourceNode[] = [];
  private cachedProgressAggregateNodes: ProgressAggregateNode[] = [];
  ...
  draw(): void {
    ...
    if (elementsVersion !== this.cachedElementsVersion) {
      ...
      this.cachedStructuredNodes = elements.filter(
        isStructuredCanvasNode
      ) as IStructuredCanvasNode[];
      this.cachedStructuredNodesSorted = [...this.cachedStructuredNodes].sort(
        (a, b) => a.zIndex - b.zIndex
      );
      this.cachedProgressSourceNodes = [];
      this.cachedProgressAggregateNodes = [];
      this.cachedStructuredNodes.forEach((element) => {
        element.appearanceAdapter = this.appearanceAdapter;
        if (isProgressSourceNode(element)) {
          this.cachedProgressSourceNodes.push(element);
        } else if (isProgressAggregateNode(element)) {
    ...
    const structuredNodes = this.cachedStructuredNodes;
    const structuredNodesSorted = this.cachedStructuredNodesSorted;
    const focusedId = this.scene.getFocusedElementId();
    structuredNodes.forEach((element) => {
      const nextCoreStates: string[] = [];
      if (element.id === focusedId) {
        nextCoreStates.push(CANVAS_INTERACTION_STATES.focused);
      }
      if (this.highlightedElementIds.has(element.id)) {
        nextCoreStates.push(CANVAS_INTERACTION_STATES.highlighted);
      }
      element.replaceInteractionStates(nextCoreStates, 'core');
    });
    ...
    structuredNodesSorted.forEach((el) => {
      if (this.semanticsAdapter.isLayoutContainer(el)) {
      ...
      if (this.semanticsAdapter.isLayoutChild(el)) {
  ...
  private updateGoalLinksAndProgressIfNeeded(connections: IConnection[]): void {
    ...
    if (this.cachedProgressAggregateNodes.length === 0) return;

    const taskIds = new Set<string>();
    const taskDoneIds = new Set<string>();
    this.cachedProgressSourceNodes.forEach((task) => {
      taskIds.add(task.id);
      if (task.status === 'done') {
    ...
    connections.forEach((connection) => {
      ...
      if (!from || !to) return;
      const goal = isProgressAggregateNode(from)
        ? from
        : isProgressAggregateNode(to)
          ? to
          : null;
      const task = isProgressSourceNode(from)
        ? from
        : isProgressSourceNode(to)
          ? to
          : null;
      if (!goal || !task) return;
    ...
    this.cachedProgressAggregateNodes.forEach((goal) => {
  ...
  private onPointerDown(e: PointerEvent): void {
    ...
    if (this.activePointers.size === 2) {
      ...
      const selectedStories = this.scene
        .getSelectedElements()
        .filter((el) =>
          this.semanticsAdapter.isLayoutContainer(el as ICanvasElement)
  ...
  private notifyPositionsDirty(elements: ICanvasElement[]): void {
    if (typeof window === 'undefined') return;
  ...
  private hasPinchChange(
    element: ICanvasLayoutContainer<IStructuredCanvasNode>,
  ...
  private captureStoryTaskPositions(
    story: ICanvasLayoutContainer<IStructuredCanvasNode>
  ...
  private getPinchMovedTasks(): IStructuredCanvasNode[] {
    ...
    const taskById = new Map(tasks.map((task) => [task.id, task]));
    const moved: IStructuredCanvasNode[] = [];
    const epsilon = 0.01;
