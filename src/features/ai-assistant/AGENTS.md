...
- Do not use border outlines on chat message bubbles of any type (assistant, user, system, action/command, typing/progress). If separation is needed, apply subtle dividers at section/container level instead.
- For action-card surfaces inside chat messages, do not apply default `HudSurface` chrome directly. Keep default cards borderless/low-chrome and use borders only for meaningful semantic states (for example failed).
