...
type AiAssistantInstructionDefinition = AiAssistantInstructionPacket & {
  summary: string;
  whenToUse: string;
  relatedToolNames: string[];
};

const DEFAULT_INSTRUCTIONS: AiAssistantInstructionDefinition[] = [
  {
  ...
  },
  {
    ...
    category: 'planning',
    title: 'AI Assistant Capability Help',
    summary:
      'Explain what this AI assistant can help with right now using frontend capability context.',
    whenToUse:
...
export class AiAssistantInstructionRegistry {
  private readonly definitions = new Map<string, AiAssistantInstructionDefinition>();

  constructor(definitions: AiAssistantInstructionDefinition[] = DEFAULT_INSTRUCTIONS) {
    definitions.forEach((definition) => {
      this.register(definition);
    });
  }

  public register(definition: AiAssistantInstructionDefinition): this {
  ...
  public getPacket(id: string): AiAssistantInstructionPacket | null {
    const definition = this.definitions.get(id);
    if (!definition) return null;
    return toInstructionPacket(definition);
  }

  public getPackets(ids: readonly string[]): AiAssistantInstructionPacket[] {
    const packets: AiAssistantInstructionPacket[] = [];
    ids.forEach((id) => {
  ...
  public listIndex(): AiAssistantInstructionIndexEntry[] {
...
export function createAiAssistantInstructionRegistry(): AiAssistantInstructionRegistry {
  return new AiAssistantInstructionRegistry();
}

function toInstructionPacket(
  definition: AiAssistantInstructionDefinition
): AiAssistantInstructionPacket {
  return {
    id: definition.id,
    category: definition.category,
