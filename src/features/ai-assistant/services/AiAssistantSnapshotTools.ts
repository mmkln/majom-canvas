...
import { buildAiAssistantCapabilityContext } from './AiAssistantCapabilities.ts';
import {
  getFocusBundle,
  getRecentActivity,
  getRelations,
...
export const AI_ASSISTANT_SNAPSHOT_TOOLS: AiAssistantToolDefinition[] = [
  {
  ...
  },
  {
    ...
    kind: 'read',
    description:
      'Returns the current AI assistant capabilities, available quick actions, AI actions, context modes, and operating constraints from the frontend runtime.',
    inputSchema: '{}',
    execute: (_input, context) => {
      const snapshot = resolveRuntimeSnapshot(context.runtime);
      const capabilities =
        context.runtime.liveHost?.getAiAssistantCapabilities?.() ??
