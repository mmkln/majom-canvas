export default class CanvasManager {
    constructor(canvas) {
        ...
        this.scale = 1; // Zoom level (optional feature)
        this.virtualWidth = 3000; // Virtual content width
        this.virtualHeight = 2000; // Virtual content height
        this.scrollbarWidth = 6; // Width/height of scrollbar
        this.draggingScrollbar = null; // Track scrollbar dragging
        this.dragStartX = 0; // Mouse X at start of drag
        this.dragStartY = 0; // Mouse Y at start of drag
        this.dragStartScrollX = 0; // Scroll X at start of drag
        this.dragStartScrollY = 0; // Scroll Y at start of drag
        this.canvas = canvas;
    ...
    drawScrollbars() {
        ...
        if (contentWidth > viewportWidth) {
            ...
            this.ctx.lineWidth = 1;
            drawRoundedRect(this.ctx, thumbX, trackY, thumbWidth, this.scrollbarWidth, this.scrollbarWidth / 2);
        }
        ...
        if (contentHeight > viewportHeight) {
            ...
            this.ctx.strokeStyle = '#5D5D5D80';
            this.ctx.lineWidth = 1;
            drawRoundedRect(this.ctx, trackX, thumbY, this.scrollbarWidth, thumbHeight, this.scrollbarWidth / 2);
        }
    }
    onMouseDown(e) {
    ...
    onMouseMove(e) {
        ...
        if (this.draggingScrollbar === 'horizontal') {
            ...
            this.scrollX = Math.max(0, Math.min(this.dragStartScrollX + deltaX * scrollRatio, scrollRange));
            this.draw();
        }
            ...
            this.scrollY = Math.max(0, Math.min(this.dragStartScrollY + deltaY * scrollRatio, scrollRange));
            this.draw();
        }
    }
    onMouseUp() {
    ...
    clampScroll() {
        const viewportWidth = this.canvas.width - this.scrollbarWidth;
        const viewportHeight = this.canvas.height - this.scrollbarWidth;
        const maxScrollX = Math.max(0, this.virtualWidth * this.scale - viewportWidth);
        const maxScrollY = Math.max(0, this.virtualHeight * this.scale - viewportHeight);
        this.scrollX = Math.min(Math.max(0, this.scrollX), maxScrollX);
        this.scrollY = Math.min(Math.max(0, this.scrollY), maxScrollY);
    }
    hitTestScrollbars(x, y) {
        const viewportWidth = this.canvas.width - this.scrollbarWidth;
        const viewportHeight = this.canvas.height - this.scrollbarWidth;
        const contentWidth = this.virtualWidth * this.scale;
        const contentHeight = this.virtualHeight * this.scale;
