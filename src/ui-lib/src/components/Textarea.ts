...
export class Textarea extends Component<TextareaProps> {
  ...
  public createElement(): HTMLTextAreaElement {
    ...
    const variantStyles: Record<'default' | 'inline', string> = {
      default:
        'rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-800 placeholder:text-slate-400 focus:border-slate-400 focus:bg-white focus:ring-2 focus:ring-slate-200',
      inline:
        'rounded-lg border border-indigo-200/70 bg-indigo-50/70 px-3 py-2 text-sm font-semibold text-indigo-700 focus:border-indigo-300 focus:bg-white focus:ring-2 focus:ring-indigo-100',
    };
